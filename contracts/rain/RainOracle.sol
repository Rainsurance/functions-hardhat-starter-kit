...
contract RainOracle is Oracle, ChainlinkClient {
  ...
  event LogRainFulfill(uint256 requestId, bytes32 chainlinkRequestId, uint256 precActual);
  ...
  constructor(
    ...
    updateRequestDetails(_chainLinkToken, _chainLinkOperator, _jobId, _payment);
  }
  ...
  function request(uint256 gifRequestId, bytes calldata input) external override onlyQuery {
    ...
    (uint256 startDate, uint256 endDate, int256 lat, int256 long) = abi.decode(
      input,
  ...
  function encodeFulfillParameters(
    ...
    return abi.encode(chainlinkRequestId, precActual);
  }
