...
contract RainOracleCLFunctions is Oracle, FunctionsClient {
  ...
  event LogRainFulfill(uint256 requestId, bytes32 chainlinkRequestId, uint256 prec, uint256 precDays);
  ...
  constructor(
    ...
    uint32 _gasLimit
  ) Oracle(_name, _registry) FunctionsClient(_chainLinkOperator) {
    subscriptionId = _subscriptionId;
    gasLimit = _gasLimit;
  }

  function request(uint256 gifRequestId, bytes calldata input) external override onlyQuery {
    (
      uint256 startDate,
      uint256 endDate,
      int256 lat,
  ...
  function fulfillRequest(bytes32 chainlinkRequestId, bytes memory response, bytes memory err) internal override {
  ...
  function encodeRequestParameters(
    ...
    return abi.encode(startDate, endDate, lat, lng, coordMultiplier, precMultiplier, secrets, source);
  }
  ...
  function encodeFulfillParameters(
    ...
    return abi.encode(chainlinkRequestId, response, "");
  }
